                ...
                @foreach( $zonasPorDia as $zona )
                    ...
                    @if( !$recorri )
                        ...
                        @foreach( $zona->rutasDelDia($diaActual) as $ruta )
                            <div class="text-left">
                                <h7 class="title text-center">Ruta : {{ $ruta->ruta_nombre  }}</h7>
                            </div>
                            <?php $detallesPorRuta = $ruta->detallesPorRuta($ruta->ruta_id);?>
                            <?php $facturasDeRuta = $ruta->facturasPorRuta($ruta->ruta_id);?>
                            @if( count($facturasDeRuta) > 0 )
                                <table class="display nowrap" cellspacing="0" width="100%" id="tableProductos">
                                    ...
                                    <tbody>
                                        @foreach( $detallesPorRuta as $detalle )
                                            <tr>
                                                <td>{{ $detalle->producto()->nombre }}</td>
                                                <td>{{ $detalle->producto()->marca()->nombre }}</td>
                                                <td>{{ $detalle->producto()->size()->nombre }}</td>
                                                <td>{{ $detalle->producto()->tipoEnvase()->nombre }}</td>
